...
void fill_truth_detection2(char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy, int rsz_w, int rsz_h)
{
    char labelpath[4096];
    find_replace(path, "images", "labels", labelpath);
    find_replace(labelpath, "JPEGImages", "labels", labelpath);
    ...
    for (i = 0; i < count; ++i) {
        ...
        if ((w < .01 || h < .01)) continue;
        if ((w * rsz_w < 8 || h * rsz_h < 8)) continue;
...
data load_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        fill_truth_detection2(random_paths[i], boxes, d.y.vals[i], classes, flip, dx, dy, 1./sx, 1./sy, w, h);
